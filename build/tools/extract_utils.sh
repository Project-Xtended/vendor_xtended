...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$XTENDED_ROOT"/vendor/xtended/build/tools/smali/baksmali.jar
        export SMALIJAR="$XTENDED_ROOT"/vendor/xtended/build/tools/smali/smali.jar
    fi
...
function extract() {
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$XTENDED_ROOT"/system_dump
